...
The next logical question is, how do we stabilize references?

## How to stabilize values

Stabilized references (currently) can be arrived at in two ways:
...
- the JavaScript language is getting native support for immutable data strcutures – [Records & Tuples]((https://github.com/tc39/proposal-record-tuple)), which are compared by **value/contents** instead of identity/reference. No need to slap `useMemo` and `useCallback` everywhere just for the sake of referential stability.
