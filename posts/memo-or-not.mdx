...
Whether or not you should memoize values in React with `useMemo` or `useCallback` is one of most commonly discussed topics in the community, and there are two use cases for memoization:
...
<details>
    ...
    <p>But even with that, immutable objects are still objects in a object-oriented programming sense. In object-oriented programming, objects have [identities](https://en.wikipedia.org/wiki/Identity_(object-oriented_programming)#Identity_and_references), as opposed to being **values** in a pure functional programming sense.</p>
     <p>For example, in Haskell, there are no objects. It only has values. Objects have identity, while values only have equality.</p>
</details>
...
- the JavaScript language is getting native support for immutable data strcutures – [Records & Tuples]((https://github.com/tc39/proposal-record-tuple)), which are compared by **value/contents** instead of identity/reference. No need to slap `useMemo` everywhere just for the sake of referential stability. 
