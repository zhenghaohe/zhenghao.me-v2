...
If a function can only take one argument of `never` type, that function can never be called (without the TypeScript compiler yelling at us):
...
Let's say we want a utility type `ExtractTypeByName` to extracts the union members with the `name` property being string literal `foo` and filter out those that don't match:
