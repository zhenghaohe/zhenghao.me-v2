...
Before the major upgrade JavaScript received in ES2015 (ES6), callbacks were how people dealt with asynchronous programming. The only way for us to express _temporal dependency_ (i.e. the execution order of asynchronous operations) is nesting one callback inside of the other. This led to so called [Callback Hell](http://callbackhell.com/).
