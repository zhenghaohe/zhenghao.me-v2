...
JavaScript commonly allows functions to work flexibly when the arguments passed are of different types and/or at different positions:
...
Let’s say we want to write a function that generates a random integer from 0-9 when called with the string `int`. Or, it generates a random English letter from a - z when called with the string `char`.
...
2. 1. Pass `T` to a generic conditional type `GetReturnType` to get the respective type for the return value.
...
> You might be wondering about the type assertion after each return statement. I’ll explain this [later](#a-note-on-the-type-assertions).
...
function getRandom<T extends 'char' | 'int' | 'bool'>(
  ...
  if (str === 'int') {
    // generate a random number
    return Math.floor(Math.random() * 10) as GetReturnType1<T>
  } else if (str === 'char') {
    // generate a random char
    ...
    return Boolean(Math.round(Math.random())) as GetReturnType1<T>
  }
}
```
...
This is to improve the soundness of the type system. See [this](https://github.com/microsoft/TypeScript/pull/30769) if you are interested in learning more about it. 
...
const promise = search("foo"); // return a promise
search("foo", (data) => render(data)); // void (return undefined)
```
...
function search<T extends Callback | undefined = undefined>(
  ...
  if (cb) {
    res.then((data) => cb(data));
    return undefined as void & Promise<Result[]>; // need to use as
  }
...
> We have to add `undefined` as a type parameter default for `T` so that the compiler can properly infer the type `T` when `cb` is not provided.
...
1. Write the overloaded function implementation.
    1. Use union types to encompass types of each of the overloads. 
    2. Within the function body,  we check the types of the arguments and dispatch the execution to a proper code path:
    ```typescript
    function switchIt_overloaded(input: string): number
    function switchIt_overloaded(input: number): string
...
function search_overloaded(term: string, cb?: Callback): void | Promise<Result[]> {
     const res = api(term)
...
function switch_overloaded(input: number | string): number | string {
     if (typeof input === 'string') {
        return input // input is still a string when it should be number
    } else {
        return input // input is still a number when it should be string
    } 
}

const num = switch_overloaded('1') // ❌ num's type is number but it is actually a string
const str = switch_overloaded(1) // ❌ num's type is string but it is actually a number
```
...
const num = switchIt_intersection(1) // ✅ of the string type
const str = switchIt_intersection('1') // ✅ of the number type
```
...
Interestingly, the usage of intersection types for function overload is a common source of confusion to people: when the overloaded function signature is written, union types are used, as opposed to intersection types.
...
When it comes to type polymorphic functions in TypeScript, I normally default to using generic types (constrained to a union type) along with conditional types. I only reach for function overload when I realize the function signature’s shapes for all its variants are different enough to be defined separately and explicitly.
