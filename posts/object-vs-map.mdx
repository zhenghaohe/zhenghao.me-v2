...
Plus, we don't have to drop supports for legacy browsers - serve legacy code via `nomodule` by serving fallback bundles so that we can avoid degrading the experience of visitors with modern browsers. Refer to [*Transitioning to modern JavaScript*](https://www.youtube.com/watch?v=cLxNdLK--yI&t=188s) if you need more convincing.
