...
In JavaScript, Object is a pretty broad term. Almost everything can be an object, except for two bottom types - `null` and `undefined`.  In this blog post, Object only refers to a collection of key-value pairs, delimited by a left brace `{` and a right brace `}`. You know, the plain old javascript objects., and there is even a acronym for those - [POJO](https://masteringjs.io/tutorials/fundamentals/pojo)

Objects are great. They allows us to easily group multiple pieces of data together. After ES6, we got a new addition to the language – `Map` . In a lot of aspects, it seems like a more capable Object. For example, a `Map` supports keys that are not strings or Symbols. On top of that, there is this common belief that `Map` is faster for operations like inserting/deleting keys at runtime.

However, most people still reach out for objects by default when they need a hash map and only switch to using `Map` when they realize the keys can’t just be strings for their use cases. As a result, `Map` remains underused in today’s JavaScript community. 

In this post, I will break down all the reasons why you should consider using `Map`s more often and when you should stick with plain objects. I also ran some benchmarks to measure their performance differences and present the results to you at the end of the post.
...
Other than that, using objects for hash maps can cause confusion and security hazards.
