---
title: Why You Might Prefer Map Over Object In JavaScript
published: true
listed: true
date: '06/18/2022'
...
- According to [my benchmarks](#performance-extravaganza), *unless* the keys are strings of small integers, `Map` is indeed **more performant** than `Object` on insertion, deletion and iteration speed, and it consumes **less memory** than `Object` of the same size. 
...
`Map` is an ES6 feature. By now most of us shouldn't be worried about its compatibility unless you are targeting some niche, old browser. By "old" I mean older than IE11 because even IE 11 supports [Map](https://caniuse.com/mdn-javascript_builtins_map) and IE 11 is [dead](https://twitter.com/swyx/status/1536353949132853248). We shouldn't mindlessly transpiling and adding polyfill to target ES5 by default, as it not only bloats your bundle size and is slow to run and penalizes 99.999% of your users that are using a normal browser that can support ES6.

Plus, we don't have to drop supports for legacy browsers - serve legacy code via `nomodule` by serving fallback bundles so we avoid degrading the experience of visitors that use modern browsers. Refer to [*Transitioning to modern JavaScript*](https://www.youtube.com/watch?v=cLxNdLK--yI&t=188s) if you need more convincing.
