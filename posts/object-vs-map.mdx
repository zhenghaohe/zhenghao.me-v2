...
listed: true
date: '06/18/2022'
description: 'why you probably should use Map more'
tags: javascript
---

> This is an unfinished draft

In JavaScript, Object is a pretty broad term. Almost everything in JavaScript can be an object, except for two bottom types - `null` and `undefined`.  In this blog post, Object only refers to a collection of key-value pairs, delimited by a left brace `{` and a right brace `}`. You know, they are just plain old javascript objects ( there is even a acronym for those - [POJO](https://masteringjs.io/tutorials/fundamentals/pojo))

Objects are great. They allows us to easily group multiple pieces of data together. After ES6, we got a new addition to the language – `Map` . In a lot of aspects, it seems like a more capable Object. For example, a `Map` supports keys that are not strings or Symbols. On top of that, there is this common belief that `Map`s are faster than objects in terms of certain operations such as inserting/deleting keys at runtime.

However, most people still reach out for objects by default when they need a hash map and only switch to using `Map`s when they realize the keys can’t just be strings for their use cases. 

As a result, `Map` remains underused in today’s JavaScript community. In this post, I will break down all the reasons why you should consider using `Map`s more often and when you should stick with plain objects. 

I also ran some benchmarks to measure their performance differences and present the results to you at the end of the post.
...
- Use `Map` for dictionaries or hash maps where you have a variable number of entries whose keys might not be known at author time, such as the map inside an event emitter. 
...
Using objects for hash maps can cause confusion and security hazards. Here are the reasons:
...
Because of prototypal inheritance, we now have two types of properties conflated: properties that live within the object itself, i.e. its *own* properties, and properties that live in the prototype chain, i.e. inherited properties. As a result, we have to rely on an additional check (e.g. `hasOwnProperty`) to make sure a given property is indeed user-provided, as opposed to inherited from the prototype. 
...
There are workarounds to prevent this. For example we can “borrow” `hasOwnProperty` from `Object.prototype` instead:
...
This is probably the most obvious downside of using objects as hash maps - objects only allows keys that are strings and symbols. Any other types will be implicitly cast to string via the `toString` method.
...
All of the above options take a runtime complexity of `O(n)`  since we have to construct an array of keys first before we can get its length.
...
Finally, the insertion order is not completely respected. In most browsers, integer keys are sorted in ascending order and take precedence over string keys even if the string keys are inserted before the integer keys.
...
Finally, we can’t rely on the dot/bracket notation to check for ****existence**** of an property because the value could be set as `undefined`

Instead we have to use `Object.prototype.hasOwnProperty` or `Object.hasOwn`
...
A `Map` is an iterable so it cooperates with `for...of` ,`forEach` and destructuring. This provides better ergonomics. You can iterate a map easily with `for...of` . And you can do things like using nested destructuring to pull out the first entry from a `Map`.
...
I first noticed `Map`s provides better performance when it comes to dynamically inserting keys at runtime when I was grinding Leetcode. 

Leetcode feeds a huge amount of data as the test cases to your solution and it times out if your solution is taking too long. Questions like [this one](https://leetcode.com/problems/random-pick-with-weight/discuss/671804/Javascript-with-explanation-and-very-interesting-find-regarding-vs-Map) only times out if you use objects, but not on `Map`s.   

I would never claim that I fully understand how V8 works under the hood to optimize `Map`s despite my many attempts to read blog posts and peek the C++ source code. Therefore, I decided to run some benchmarks to measure the performance characteristics between objects and `Map` s for various operations. 

Perfectly robust benchmarking is hard. Most of us never went through any form of training in either benchmarking or interpreting the results. The more benchmarking I do the more it felt like a story about [blind men and an elephant](https://en.wikipedia.org/wiki/Blind_men_and_an_elephant). So take everything I said here about performance a grain of salt. You’ll need to test such changes with your application in a production environment to know for sure if there are actual performance gains from using `Map`s over objects.

## Preparation

I built [a simple app](https://codesandbox.io/s/still-glitter-yuu1dm?file=/src/index.js) to run the benchmarks. It has a table that shows the numbers of operations per second for insertion, deletion and iteration on objects and maps.
