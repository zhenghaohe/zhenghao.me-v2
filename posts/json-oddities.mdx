...
1. Since string inputs will be wrapped in double quotes ", (e.g. `JSON.stringify('3') === '"3"'`), in order to in order to avoid ambiguity, `JSON.stringify` will escape the ones present in the original input using backslashes, such as ` JSON.stringify('"3"') === '"\\"3\\""'`.
...
function jsonStringify(data) {
  ...
  if (type === 'string') return quotes + data.replace(QUOTE_ESCAPE, '\\"') + quotes;
  ...
  if (type === 'object') {
    ...
    if (data instanceof Array) {
      return '[' + data.map((item) => jsonStringify(item)) + ']'; // here Array.prototype.toString will be invoked implicitly during string concatenation
    }

    // data is a plaid object
