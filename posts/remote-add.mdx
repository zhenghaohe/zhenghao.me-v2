...
--------------------
Here we use `setTimout` to simulate the API call.
...
A better approach would be to perform the async additions in parallel to speed up the process with a parallel approach.
...
<details>
    ...
    <p>In this case, since we are switching to an approach with making multiple requests in parallel, it is important to understand if our `remoteAdd` API is **rate-limited** or not. Most of API providers will limit the number of requests within any given second so sending too many requests in quick succession may trigger rate limiting and result in a [status code 429](https://developer.mozilla.org/en-US/docs/Web/HTTP/Status/429). So check with the interviewer on this before continuing.</p>
</details>
...
add(1,2,3,4,5,6,7,8,9).then(sum => console.timeEnd('parallel'))
// 413.84619140625 ms
```
...
1. since addition has associtiy and communiv , `addRemote(1,2)` and `addRemote(2,1)` give us the same result even though the inputs are different. Our cache needs to account for that. 
