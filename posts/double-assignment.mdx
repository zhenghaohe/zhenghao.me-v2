---
title: the double-assignment pattern in JavaScript
published: true
listed: true
date: '01/22/2022'
tags: javascript,v8
...
A few years ago, when I just started learning to program, I was looking at the source code of jQuery since it is the most mature JavaScript library out there - until this date, the frontend of amazon.com is still [powered by it](), and I wanted to learn from the best. In its source code, I distinctively remember there were code written like this (I omitted things for simplicity. You can check out its source code):
...
1. First, (the past) `elemData` pointing to `initialValue` , and (the past) `elemData.events` property pointing to the value of the assignment expression `elemData = function(){}`
...
while (i < 10) {
  node.next = node = {} // `node` in `node.next` is the past `node`
