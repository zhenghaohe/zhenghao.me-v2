...
Technically this is impossible to achieve. Instead, TypeScript makes `never` an empty type (a.k.a an uninhabitable type): a type for which we cannot have an actual value at runtime. The canonical example where such a type is fitting is when we want to type a return value from a function that never returns.
