...
tags: javascript
---
Memory leak has remained a relatively underdiscussed topic in the JavaScript community, largely because client-side JavaScript shipped with web pages to browsers, compared with their server-side counterpart, are typically short-lived. Most of the times, a web page doesn't stay open for extended periods of time and once the user refresh the page or close the tab, memory retained by JavaScript all get automatically released and recycled.
