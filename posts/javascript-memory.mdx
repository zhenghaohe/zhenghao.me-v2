...
## TL;DR:
1. All JavaScript values are allocated on the heap accessed by pointers no matter if they are objects, arrays, strings or numbers (except for small integers i.e. `smi` in [V8](https://v8.dev/blog/react-cliff#value-representation) due to [pointer tagging]([https://v8.dev/blog/pointer-compression#value-tagging-in-v8](https://v8.dev/blog/pointer-compression#value-tagging-in-v8))).
...
![alt](/art/blog/javascript-memory/before.jpg)

After I create a string of a size of 10 MB, the heap memory used increases to 13.78 MB
![alt](/art/blog/javascript-memory/after.jpg)
...
![alt](/art/blog/javascript-memory/memoryProfile.jpg)
...
![alt](/art/blog/javascript-memory/onlyOneString.jpg)
...
![alt](/art/blog/javascript-memory/oddballs.jpg)
...
When we are creating JavaScript variables that "have" `Oddball` values, we should think as if they are summoned in our JavaScript program - we cannot create or destroy them.
...
In V8, integers ranging from -2³¹ to 2³¹-1 on a 64-bit architecture (the V8 term is `smi`) are [heavily optimized](https://v8.dev/blog/pointer-compression#value-tagging-in-v8) so they can be encoded inside of a pointer directly without the need to allocate additional storage for it.
...
For `smi`s, they are encoded as recognizably invalid pointers, which don't point to anything, so the whole concept of "reusing" doesn't really apply to them.
When `HeapNumber`s (numbers that are not considered `smi`s) are pointed by object's properties, it becomes a mutable `HeapNumber`, which allows updating the value without allocating a new `HeapNumber` every time. I believe this design decison is a beneficial tradeoff for the majority usage patterns at the cost of making `HeapNumber`s unshareable.
...
![alt](/art/blog/javascript-memory/smi.jpg)
...
![alt](/art/blog/javascript-memory/overview.jpg)
