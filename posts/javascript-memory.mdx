...
For numbers that are not considered `smi`, they are called `HeapNumber`. When a `HeapNumber` is pointed by an object's property, it becomes a mutable `HeapNumber`, which allows updating the value without allocating a new `HeapNumber` every time. I believe this design decison is a beneficial tradeoff for the majority usage patterns at the cost of making `HeapNumber`s unshareable.
