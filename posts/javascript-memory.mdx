...
You can tell that two `smi`s are "pointing" to the same memory location `@427911` - that is because they have the same bit pattern for the same value `123`, and Chrome DevTools still treats them as pointers even though they are invalid pointers due to pointer tagging. 

As to `HeapNumber`s, they are pointing to the different memory locations `@427915` and `@427927`, meaning they are not reused.
