...
listed: true
date: '12/23/2021'
description: walk through of similarities and differences between frontend vs. backend system design interviews and frontend career ceiling
tags: technical interviews,muse
---

## Context

As I mentioned in [my last post](zhenghao.io/posts/framework-for-coding-interview), I have been interviewing a lot for the past year - not conducting interviews, I was the candidate being interviewed. Most of the interview processes I had with big tech companies consisted of 1-2 system design rounds depending on the level of the role. Unlike traditional **backend-focused** system design questions, for which you can find lots of prep resources online such as the famous [grokking system design interview](https://www.educative.io/courses/grokking-the-system-design-interview)

or [system design primer](https://github.com/donnemartin/system-design-primer), I didn’t know what to expect for a **frontend-focused** system design interview as there are very few resources out there talking about this type of interviews.
...
Both frontend-focused and backend-focused system design interviews share a lot of similarities in terms of how you approach it methodically: 
...
1. The interviewer is relying on you to drive the presentation. You can't to rely on the interviewer to have your back.
...
3. Both types will consist of open ended questions are not like scantron school exams. There probably isn’t a checklist of things for you to cover one by one. Nor are you expect to drill deep into all of those. When you realize your interviewer is biased toward a particular part of the system, pivot your focus to that area. Other times you focus on your strengths and lead the converstation.
...
4. Choosing the right the cross-region strategy if your service has a global userbase
<br />

For frontend-focused system design interviews, you would spend most of the time talking about stuff like:

- frontend/client side architecture, such as which rendering pattern to choose - client side rendering, or server side rendering or static generation or something in between?
...
- Specifics about UI components
    - a newsfeed which has an infinite scroll behaviour with all the images lazily loaded while ensuring the client side has the aspect ratio upfront to prevent layout shift.
    - an autocomplete UI component which also receives images from server pushes in parallel with fetching search result data.
- How do you leverage different layers of cache to decrease latency or support offline mode.
- if they want to get framework specific, which is totally possible, they might even ask you to define a particular React components’s props.

<br />

A lot times when having one type of system design interviews, you can over simplify the opposite side: 
...
- In a frontend system design interview, you can treat the backend as a black box and you don’t need to worry about how things like how to scale a database, or how your choice of using websockets might affect the load balancers because of the need for sticky sessions support.
<br />
...
2. The two types of system design interviews also differ a lot in terms of *estimating* certain system's needs based on the potential scale of the system. The system needs can be storage needs or throughput needs or any other types of requirements. 
...
## Career Ceiling
This is a little bit tangential but I do want to talk about this. 
...
This is an unspoken thing and a very impolite conversation. Part of the ceiling comes from some traditional baggage 
