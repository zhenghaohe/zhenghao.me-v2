...
- There are **no** built-in ways to check data types in JavaScript that are straightforward and all-encompassing. All of the current native ways are imperfect (or just straight up buggy).
...
One fun fact about `typeof`: it is one of the _only_ two operators in existence of the language that is able to reference a thing that doesn’t exist and _not_ throw an error (the other is the `delete` operator). You can get away with something like this:
...
Turns out there is a third, arguably better way to check data types in JavaScript – `Object.prototype.toString`. It is a method that exists on `Object.prototype`, which returns a string value used for the description of an objects based on its `Symbol.toStringTag`.
...
In fact, many runtime type validation libraries use this technique underneath, such as [this one](https://github.com/medikoo/type/blob/main/set/is.js#L7).
