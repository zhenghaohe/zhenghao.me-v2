...
> Using object literals always gives you new, **unique** objects, while primitive values (except for numbers, they are comlicated) are **reused**. That means with `const a = 'foo'; const b = 'foo'`, we only have one string `foo` stored in the memory at runtime due to [string interning](https://en.wikipedia.org/wiki/String_interning).
