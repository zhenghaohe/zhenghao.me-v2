...
1. if we stay at the JavaScript language specification level, [as I pointed out early](#what-does-the-spec-say), you cannot bind a variable identifier with a Reference Record Specification type. This prevents you from having a variable that has the **location** of another variable as its value. 
2. Going deeper into the implementation level, there is no way to get a variable’s memory address. If `a` is loaded at `0x100`, we cannot get that address `0x100` and store it inside of another variable `b` and later dereference `b` to access and change data contained the memory block of variable `a`.
