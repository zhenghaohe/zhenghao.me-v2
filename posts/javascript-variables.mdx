...
1. stay at the language specification level, discuss different types of mental models around variables
...
3. Objects are not passed by reference. You might argue it comes down to nomenclature, but in JavaScript passing objects to a function, or primitive values to a function are not handled differently in memory.
...
Therefore, variables in C/C++ are like boxes, unless you declare them a reference/pointer.

### a wire
The “box” mental model fails when the variables and the values bound to them are stored in different places in memory. 

```
const foo = {pos1: {x: 1, y: 2}, pos2: {x: 3, y: 4}};
```
Unlike the C++ example above, `foo` in JavaScript does not contain the object value inside. There is an extra layer of indirection here. 
...
Given the code snippet above, will we end up with two separate memory chunks that *duplicately* make up the string `baz`? Or there is only one string `baz` at runtime which is pointed by both variable `foo` and `bar`? 

The answer is the latter. It is not much different than objects, except that objects are immutable and primitive types are not. 
![alt](/art/blog/javascript-variables/JSPrimitiveExample1.png)

There is another difference between object values and primitive values - using object literals always gives you new, unique objects, while primitive values are **reused**. 
What that means is that `const a = {}; const b = {}` gives us two different object value `{}`, but with `const a = 'foo'; const b = 'foo'`, we only have one string `foo` stored in the memory at runtime. Same thing with other types of primitive values like number (except for small integers), booleans, `undefined` etc.. This might seem strange to you at first. I will prove it to you in a separate blog post.

Having the right mental modal, the right approximation can have a huge positive impact on our day-to-day programming, even if the mental modal itself is deceptively simple and intuitive. Remember, JavaScript variables do not have or store values - they only **point** to or **refer** to values.
...
For example: when you write an assignment like `a = b`, it gets evaluated, and two References for variables `a` and `b` get constructed (to be precise, `a` and `b` are identifiers). The Reference of the right-hand side (RHS) of the equation gets passed to this abstract function called [GetValue](https://tc39.es/ecma262/#sec-getvalue), which fetches the value bound to/pointed by the Reference. Then an abstract [PutValue](https://tc39.es/ecma262/#sec-putvalue) is invoked with the obtained value from RHS and the Reference of the left-hand side (LHS), setting the new binding via `SetMutableBinding`. Note that the left-hand side remains a Reference during the assignment since the engine needs to know where to store the value, while the right-hand side is always evaluated as a value via `GetValue`. You can't put the Reference of RHS to the Reference of LHS. That's precisely why you cannot have a variable pointing to another variable - a wire only connects you to a value.

One of the only two operators which can operate directly at a Reference is `delete`, (the other is `typeof`). When you use `delete` to delete a property in an object, you are not delete the value pointed by the property. Instead, you are merely deleting the Reference, i.e. the location at which the value is stored.

I will be the first to admit that the spec is hard to read. It is not written from the perspective of a language tutorial. Instead, it is written from the point of view of how to write a javascript interpreter. The point I am trying to make about the Reference from the spec is that, even by staying at the abstraction level of the language itself without peeking into its implementation, we can still form the right mental modal on JavaScript variables and be an effective JavaScript developer.

## What are JavaScript variables made of exactly
The topics discussed below are all considered “implementation details”. This is where the language specification ends, and the implementation starts.

> I am going to explain what JavaScript variables really are from the perspective of the JavaScript engine - V8. There are different JavaScript engines in the wild. And even just for V8 along, it is very complex, and it has multiple ways to run code, and its various parts and pipelines have been rewritten many times over the years. What I describe today might not be outdated tomorrow. Don't rely on the implementation details.

Let’s step back for a second. At its simplest, think of a variable as an identifier, an alias to the chunk of memory at which its immediate value is stored. At compile time, Your variables are all turned into the memory addresses. Wherever that variable is used in the code, its (immediate) value will be accessed via that memory address. 

As we have discussed, the values we see in our code are not directly stored in JavaScript variables. At a low level, that means the bits from the memory block of which a JavaScript variable is an alias does not make up its value directly. Then what exactly is stored in a JavaScript variable? The answer, it stores a memory address of some other memory block. Starting at that address, there is a separate chunk of memory, at which a real value, the value we see, manipulate and pass around in our JavaScript program, is stored.
...
Therefore, it is technically incorrect to say that “variable `a` has the empty object as its value ” - variable `a` only has the memory address of that object.

### Everything is on the heap

We haven't talked much about primitive values. For V8 currently, values of primitive types, except for integers ranging from 2³¹ to 2³¹-1 on a 64bit architecture (more on this later), are just like objects - they are stored separately, and the JavaScript variables can only point to them. 
...
However, this is an oversimplification. I am planning on writing another blog post about the memory layout in V8.


## The hidden layer in JavaScript

The second table (blue) in the previous diagrams is always hidden in the JavaScript world, just like our Reference Record Specification Type. Within JavaScript, you cannot see or interact with memory addresses except for retrieving the actual values pointed by them, nor can you get the memory address of that memory address itself either. Mouthful right? Interacting with your memory in a low-level way can get really tricky, so the JavaScript engine manages that for you. 

For example, when you place an assignment operator = between two variables in `a = b`, you don’t really need to think about scenarios like this as it cannot happen in JavaScript:

![alt](/art/blog/javascript-variables/hiddenLayerInJS.png)

Now you know what JavaScript variables are made of - they are (directly) made of memory addresses, pointing to where the values you create and manipulate in your JavaScript program are actually stored. But don't go overboard and use pointers to refer to every JavaScript variable you declared. Unlike C/C++, for JavaScript, the pointer is not a term that gets used in the language semantics. Calling variables pointers would not add any useful distinction.

## Boxing

You might be thinking: why do we have to add extra pointer indirections between values and variables/object properties? 
This (most likely) has to do with the nature of the language is dynamically typed. In a dynamically typed programming language, the type checks are performed at runtime. However, the variables can hold any type of value at runtime. Therefore the types cannot be associated with a variable but with the underlying value pointed/referenced by the variable. To achieve that, we need a uniform memory representation for JavaScript variable - a pointer. And the real values are allocated somewhere on the heap(dynamic memory), containing metadata such as the type and the size of the value, pointed by those pointers. 
Wrapping primitives types inside another data structure that records extra metadata about the value is known as boxing.

## small integers
And I have been mentioning that small integers are an exception to this boxing rule. Small integers are heavily optimized so they can be encoded inside of a pointe. Unlike every other type, a small integer can be stored directly in the pointer without allocating additional storage for it.

This means, it is technically to say that `a` has the value `123` as in `const a = 123`:
![alt](/art/blog/javascript-variables/smiExample.png)

Read this article from V8 if you are interested in how they distinguish between a small integer vs. a pointer for an immediate value. This technique is called [pointer tagging](https://v8.dev/blog/pointer-compression#value-tagging-in-v8), and it is not unique to V8 or JavaScript. A bunch of other languages like [OCaml](https://dev.realworldocaml.org/runtime-memory-layout.html) and [Ruby](https://brandur.org/ruby-memory#value) does this too.
...
2. Therefore, we can say a JavaScript variable or an object property has the value only when the value is a small integer. Otherwise, they only point to values.


## Why JavaScript can not do pass-by-reference

This myth is not unique in the JavaScript community. I have seen this come up in [the Java community](https://www.javadude.com/articles/passbyvalue.htm) as well.
...
At its core, for JavaScript to support pass-by-reference, we need to have a way to change the value of a given variable without doing a re-assignment.
...
```javascript
let a = 1
// ... any thing another than `a = 2`
console.log(a) // if a can be changed to `2` then JavaScript can do pass-by-reference
```

You cannot. It is impossible in JavaScript:
1. if we stay at the JavaScript language specification level, as I pointed out early, you cannot bind a variable identifier with a Reference Record Specification type.
2. Going deeper into the implementation level, there is no way to get a variable’s memory address. If `a` is loaded at `0x11`, we cannot get that address and store it inside of another variable `b`.

You might argue things would change if `a` points to an object. For example, if we declare variable `a` as `const a = {}`, there are indeed ways to modify the object without directly reassigning `a` a new value. 

In fact a lot of people get the wrong impression when passing an object to a function: within that function, it is true we can mutate the object, and that mutation would reflect on the object value passed from the outside. An example would be:
...
In fact, this is exactly pass-by-value - the value that gets passed and copied is the memory address of the object `{}`.

![alt](/art/blog/javascript-variables/passbyvalueExample.png)

> Maybe another more fitting or intuitive name for how object arguments are passed and evaluated should be [pass-by-share](http://dmitrysoshnikov.com/ecmascript/chapter-8-evaluation-strategy/#call-by-sharing)
